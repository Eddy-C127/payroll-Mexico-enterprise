...
class HrPayrollDispersion(models.TransientModel):
    ...
    def compute_sheet(self):
        ...
        for payslip_id in self.payslip_run_id.slip_ids:
            if (
                not payslip_id.employee_id.address_id
                or not payslip_id.employee_id.address_id.bank_ids
                or not payslip_id.employee_id.address_id.bank_ids.filtered(
                    lambda acc: acc.bank_id == self.bank_id
                )
            ):
                continue
            acc_number = payslip_id.employee_id.address_id.bank_ids.filtered(
                lambda acc: acc.bank_id == self.bank_id
            )[0].acc_number
            data = [""] * 9
