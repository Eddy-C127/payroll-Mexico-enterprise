...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_mx_hr_payroll_edi_check_configuration(self, payslip):
        ...
        certificate = (
            company.l10n_mx_hr_payroll_edi_certificate_ids.sudo().get_valid_certificate()
        )
        if not certificate:
    ...
    def _l10n_mx_hr_payroll_edi_get_common_cfdi_values(self, payslip):
        ...
        currency_precision = payslip.currency_id.l10n_mx_edi_decimal_places
        customer = payslip.contract_id.employee_id.address_id
    ...
    def _l10n_mx_hr_payroll_edi_get_payslip_cfdi_values(self, payslip):
        ...
        cfdi_values["total_cfdi"] = cfdi_values["total_allowance_amount"] - (
            cfdi_values["total_deduction_amount"]
        )
    ...
    def _l10n_mx_hr_payroll_edi_export_payslip_cfdi(self, payslip):
        ...
        # == CFDI values ==
        cfdi_values = self.sudo()._l10n_mx_hr_payroll_edi_get_payslip_cfdi_values(payslip)
        (
            qweb_template,
            xsd_attachment_name,
        ) = self.sudo()._l10n_mx_hr_payroll_edi_get_templates()

        # == Generate the CFDI ==
        cfdi = self.env["ir.qweb"]._render(qweb_template, cfdi_values)
        decoded_cfdi_values = payslip.sudo()._l10n_mx_hr_payroll_edi_decode_cfdi(
            cfdi_data=cfdi
        )
        cfdi_cadena_crypted = (
...
""" % {
    ...
    def _post_payslip_edi(self, payslips, test_mode=False):
        ...
        for payslip in payslips:
            ...
            # == Generate the CFDI ==
            res = self.sudo()._l10n_mx_hr_payroll_edi_export_payslip_cfdi(payslip)
            xml_temp = res
            base64_xml_temp = base64.encodebytes(xml_temp['cfdi_str'])
            if res.get("errors"):
                edi_result[payslip] = {
                    "error": self._l10n_mx_hr_payroll_edi_format_error_message(
                ...
                cfdi_faile_attachment = self.env["ir.attachment"].create(
                    {
                        ...
                        "type": "binary",
                        "datas": base64_xml_temp,
                        "mimetype": "application/xml",
                        "description": _(
                            "Mexican Failed payslip CFDI generated for the %s document."
                        )
                        % payslip.name,
                    }
                )
                payslip.message_post(body="Failed nomina", attachment_ids=cfdi_faile_attachment.ids)
            ...
            # )
                ...
                payslip.message_post(body=xml_error['error'])
                continue
            else:
    ...
    def _l10n_mx_edi_format_error_message(self, error_title, errors):
        bullet_list_msg = ''.join('%s' % msg for msg in errors)
